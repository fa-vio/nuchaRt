...
private:
	...
	void useOnlyChrom(size_t chr) {
		size_t f_p, f_q, stop, i, hs;
		std::vector<SamData> ns;

		Rcpp::Rcout << "Using only fragments on Chromosome " << chr << std::endl;
	...
	void kCores(Graph *meg, std::string unm);
...
public:
	...
	~Finder() {
	    ...
	    sam->clear();  // clear vector before freeing its memory
	                   // otherwise it would not release memory to OS 
	    if(pnp) {
#ifdef USE_NUMA
	        if(numa_available >= 0)
	...
	// parsers
	void parseFiles(std::string genefile, std::string fragfile, std::string samfile);
    void reducedParseFiles(std::string samfile, std::string featfile);
    void parseExpressions(std::string exprFile);
	...
	int parFindConnections( std::vector<std::string> gvec, std::vector<Edge>& r_edges,
                        std::string dts, int sc_limit, int w, std::string clstr, int chunk, 
                        float e_lm, int plot, int colls, int exprs, std::string uname, 
                        std::string locs="", bool active=false, int only_chr=0 );
	void csStdDev(std::string dirPath);
	size_t getMaxFeatQtValue() { return maxMapValue(feat_qt); }
	std::vector<SamData>* samVector() { return sam; }
...
private:
	...
	umCM cmaps; //std::unordered_map<uint_64, CMap<int> > cmaps; // TODO
};
