...
public:
	 ...
	 */
	GeneT() :
		Vertex(), start_p(0), stop_p(0), hs(0), conn(NULL), cis(0), trans(0), nexp(0) {
	}
	...
	GeneT(uint_64 st, uint_64 sp, long eid, int lv, size_t h, uint_64 est, uint_64 esp,
			long pos, int cis_, int trans_, int nexp_) :
		Vertex(), start_p(st), stop_p(sp), exst(est), exsp(esp), hs(h), conn(NULL), cis(cis_), trans(trans_), nexp(nexp_) {
		setId(pos);
		Vertex::level = lv;
	}
    ...
    inline long getNexp() 			const { return nexp; }
