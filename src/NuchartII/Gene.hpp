...
struct Node {
	Node(std::string lbl, long id, long dg, long intrs, size_t hs_, int cis_, int trans_, int l_,
			bool rt, bool it, double gxp, int nxp) :
		label(lbl), n_id(id), deg(dg), intrs(intrs), cis(cis_), trans(trans_), lvl(l_), nexp(nxp), hs(hs_),
...
/**
 * This class describes a Gene object, which is a Vertex in my graph.
 */
...
public:
	 ...
	 */
	Gene() :
		Vertex(), start_p(0), stop_p(0), entrez_ID(-1), hs(0), intr(0), cis(-1), trans(-1), nexp(0) {
	...
	Gene(const Gene& g) : Vertex(g),symbol(g.symbol), chrom(g.chrom),
			start_p(g.start_p), stop_p(g.stop_p), entrez_ID(g.entrez_ID), hs(g.hs), intr(g.intr), nexp(g.nexp),
	...
	Gene(Gene&& g): Vertex(std::move(g)), symbol(std::move(g.symbol)), chrom(std::move(g.chrom)),
			...
			entrez_ID(std::move(g.entrez_ID)), hs(std::move(g.hs)), intr(g.intr), cis(g.cis), trans(g.trans), nexp(g.nexp),
	...
	inline int getNexp() 			    const { return nexp; }
	inline long getPosition()			const { return getId(); }
	inline double getGeneExp() const { 
 	    if(exp_t.empty()) return 0.0;
 	    else {
 	        double sum = std::accumulate(exp_t.begin(), exp_t.end(), 0.0,
                                       [](double current_sum, const Expression& exp) {
                                           return current_sum + exp.logfc;
                                       });
 	        return (sum / exp_t.size());
 	    }
	}
	...
	inline void setExpression()		                { nexp++; }
	...
	GeneT* toGeneT() {
		GeneT *gt = new GeneT(start_p, stop_p, entrez_ID, getLevel(), hs,
				gene_contacts.start, gene_contacts.stop, getPosition(), cis, trans, nexp);
		return gt;
	}
