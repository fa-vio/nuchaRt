...
// parses only files needed for normalisation phase - reduced memory consumtion
void Finder::reducedParseFiles(std::string samfile, std::string featfile) {
    ...
    Parser ps;
    ps.scanSamFile(samfile, *sam);
    ps.parseFeaturesFile(uniques, featfile);
    
    samNumbers();
    featuresNumbers();
}

void Finder::parseExpressions(std::string exprFile) {
    Parser ps;
    ps.parseExpressionFile(expressions, exprFile);
}
...
size_t Finder::buildCMaps() {
    if(uniques.empty()) {
        Rcpp::stop("ERROR - Features file is empty!");
        //exit(EXIT_FAILURE);
    }
...
void Finder::findGeneExpression(Gene& g) {
    if(!expressions.empty()) {
        for(unsigned u=0; u<expressions.size(); ++u)
            if( expressions[u]->hs == DJBHash(g.getSymbol()) ||
                (expressions[u]->eid == g.getEID()) )
                g.pushExpression(*expressions[u]);
            g.sortExpressions();
    } else {
        Rcpp::stop("ERROR - Expression file is empty.");
...
void Finder::findBindings(Gene& g) {
	if(!beds.empty()) {
	    ...
	    Rcpp::stop("ERROR - Bed file is empty.\n");
...
Gene Finder::getSingleGeneByEntrezId(int id) {
	if(genes.empty()) {
		Rcpp::stop("Genes file empty! Aborting...");
		//exit(EXIT_FAILURE);
	}
	...
	if(found.size() == 0) {
		Rcpp::Rcout << "Gene " << id << " not found." << std::endl;
	    Rcpp::stop("Process terminating.");
		//return g; // exit(EXIT_FAILURE);
	}
...
Gene Finder::getSingleGeneBySymbol(const std::string sym) {
	...
	if(found.size() == 0) {
		Rcpp::Rcout << "Gene " << sym << " not found." << std::endl;
	    Rcpp::stop("Process terminating.");
		//return g; // exit(EXIT_FAILURE);
	}
...
std::vector<Gene> Finder::getGenesByCoordinates(std::string chr_, uint_64 gst, uint_64 gend) {
	...
	if(found.size() == 0) {
		Rcpp::Rcout << "Genes in gene cluster not found." << std::endl;
	    Rcpp::stop("Process terminating.");
		//return found; // exit(EXIT_FAILURE);
	}
...
int Finder::parFindConnections(std::vector<std::string> gvec, std::vector<Edge>& r_edges,
                            std::string dts, int sc_limit, int w, std::string clstr, int chunk, 
                            float e_lm, int plot, int colls, int exprs, std::string uname, 
                            std::string locs, bool activeSched, int only ) {
	...
	if(frags.empty())
	    Rcpp::stop("ERROR: Fragments vector is empty!\n");
	...
	for(unsigned gv=0; gv<gvec.size(); ++gv) {
		...
		if(root->getExtendedStart()) {
			...
			Rcpp::Rcout << "WARNING: fragments not found for Gene "
					<< gvec[gv] << ". Skipping gene..." << std::endl;
		}
	}
	// ----------------------------------------------------------------
	
	if(only && gqueue.size() == 1) {
	    useOnlyChrom(gqueue[0]->getHS());
	    sam_size = sam->size();   
	}
	...
	if(gqueue.empty()) {
		//Rcpp::Rcout << "ERROR: chromosome fragments not found for starting genes. Function returns." << std::endl;
	    //exit(EXIT_FAILURE);
	    Rcpp::stop("ERROR: chromosome fragments not found for starting genes. Function returns.\n");
	}
	...
	while(!gqueue.empty() && level < sc_limit) {
		...
		Rcpp::Rcout << "Level " << level << " with " << gqsize << " node(s)" << std::endl;
		...
		if(tasklist.empty()) {
		    //Rcpp::Rcout << "No connections found to start the computation. Function returns." << std::endl;
		    //exit(EXIT_FAILURE);
		    Rcpp::stop("No connections found to start the computation. Function returns.");
		}
		// ----------------------------------------------------------------------------------------

		Rcpp::Rcout << "Found " << tasklist.size() << " connections ("
		...
		ffpf.parallel_for_thid( 0, conn_size, 1, chunk, [&, this] (const unsigned i, const int thid) {
			...
			for (; f_qgene > 0; ++f_pgene, --f_qgene) {
				if (genest[f_pgene]->getExtendedStart() <= r1->getStart2()
					...
					localGraphs[thid].push_back( new EdgeT((tasklist[i]->w_gene), genest[f_pgene], 
                                            tasklist[i]->conn, /*NULL,*/ F1st, F1end, F2st, F2end, 
                                            false) );
				}
			}
		...
		red2 += red;
		Rcpp::Rcout << " Built partial Graph at level " << level << " (" 
		            << epf << " + "<< red << "ms)" << std::endl;
	...
	Rcpp::Rcout << " -- STOP FINDING NEIGHBOURS: " << st1 << "ms." << std::endl;
	Rcpp::Rcout << "\n-- TIMINGS:\n"
...
// 	delete p;
		...
		if(v_mask[v]) {
			...
			if(genes[v].getLevel() == 0) {
				t_vertex.push_back( Node( genes[v].getSymbol(), genes[v].getPosition(),
						bigGraph->getNodeDegree(genes[v].getPosition()), genes[v].getIntr(), 
						genes[v].getHS(), genes[v].getNumCis(), genes[v].getNumTrans(), 0, true, 
						false, genes[v].getGeneExp(), genes[v].getNexp() ) );
			} else {
				t_vertex.push_back( Node(genes[v].getSymbol(), genes[v].getPosition(),
						bigGraph->getNodeDegree(genes[v].getPosition()), genes[v].getIntr(), 
						genes[v].getHS(), genes[v].getNumCis(), genes[v].getNumTrans(), genest[v]->getLevel(), 
						false, false, genes[v].getGeneExp(), genes[v].getNexp() ) );
	...
	if(numa_available >= 0) {
	    ...
	    edgs = new (ept) std::vector<Edge*>(tsz);
	} else {
	...
	//ffpf.parallel_for(0, tsz, 1, chunk, [&] (const unsigned i) { // SEGFAULTS WITH PARFOR WITH NWRKS > 1
	for(unsigned i=0; i<tsz; ++i) {
	    uint_64 cst1 = t_edges[i]->conn1->getStart1();
	    uint_64 cst2 = t_edges[i]->conn1->getStart2();
		...
		for(; f_q > 0 && !found_r2; ++f_p, --f_q) {
			if( (cst1 == samt->at(f_p)->getStart2()) &&
					(cst2 == samt->at(f_p)->getStart1()) ) {
                ...
                try {
                    edgs->at(i) = new Edge( t_edges[i]->getVertex1(), t_edges[i]->getVertex2(),
                             ...
                             t_edges[i]->getHashedValue(), 0, 0, false);
                } catch(const std::out_of_range& e) {
                    Rcpp::Rcout << "[ERROR] " << e.what() << std::endl;
                }
                
                r_edges.push_back( Edge(t_edges[i]->getVertex1(), t_edges[i]->getVertex2(),
                        genes[ids[2*i]].getSymbol(), genes[ids[2*i+1]].getSymbol(),
                        sam->at(t_edges[i]->conn1->getId()).getSeq(), sam->at(s2id).getSeq(),
                        sam->at(t_edges[i]->conn1->getId()).getChr1(), sam->at(t_edges[i]->conn1->getId()).getChr2(),
                        t_edges[i]->getHS1(), t_edges[i]->getHS2(), t_edges[i]->getStart1(),
	...
	delete[] ids;
	Rcpp::Rcout << "-- DONE IN " << rest << "ms." << std::endl;
	// ------------------------------------------------------------------------------------------
	...
	int d=-1;
	Rcpp::Rcout << " - Creating folders..." << std::endl;
	
	std::string f_out("/home/"), tm(tmn::timeStamp());
	f_out.append(uname).append("/NgrapH/csv");
	...
	Rcpp::Rcout << " \nOutput files written in \'" << f_out << "\'" << std::endl;
	Rcpp::Rcout << " Printing Core decomposition (deterministic)..." << std::endl;
	kCores(bigGraph, uname);
...
// mean and standard deviation among genes' cis and trans values
void Finder::csStdDev(std::string dirPath) {
	...
	if(files_sz < 2) {
		Rcpp::Rcout << "ERROR: needed at least two files to compute "
 ...
 * Creates an instance G' of the graph 'g' and iteratively improves G' by 
 * replacing edges such that the total discrepancy is reduced. Results in 
 * a representative instance of the input graph.
 * 
 * see: The Pursuit of a good Possible World: Extracting Representative 
 * Instances of Uncertain Graphs, P. Parchas, F. Gullo, D. Papadias, F. Bonchi.
 * 
 */
void Finder::ADR(Graph *g, int p) {
	...
	meg->printGraph(Rcpp::Rcout);
	Rcpp::Rcout << "MaxDegree: " << meg->getMaxDegree()
 ...
 * The k-core of input graph 'meg' is the maximal subgraph 'H' of 'meg' such 
 * that every vertex in 'H' has degree at least equal to k.
 * The set of all k-Cores of 'meg', for all k, is the core decomposition of 
 * 'meg'.
 */
void Finder::kCores(Graph *meg, std::string unm) {
	Graph *g = meg;
	...
	Rcpp::Rcout << " K-cores computed in " << elp << "ms." << std::endl;

	// print kcores to file
	std::string kc_folder("/home/");
	kc_folder.append(unm).append("/NgrapH/kCores");
	...
	std::cout << " K-cores decomposition written in \'" << ess.str() << "\'" << std::endl;
...
/*
 * - Probabilistic (k,n)-Core decomposition -
 * The probabilistic (k,n)-core of an uncertain input graph 'meg', with a 
 * threshold n in [0,1], is the maximal subgraph 'H' of 'meg' such 
 * that the probability that each vertex in 'H' has degree no less than k in 
...
void Finder::ketaCores(Graph *meg, double eta) {
	...
	Rcpp::Rcout << "Keta-cores computed in " << elp << "ms." << std::endl;
